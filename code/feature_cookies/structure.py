...
from .utils import ad_keyword_ascendants
...
def get_script_content_features(G, df_graph, node, ldb):
    ...
    keywords_fp = [
        "CanvasRenderingContext2D", "HTMLCanvasElement", "toDataURL", "getImageData",
        "measureText", "font", "fillText", "strokeText", "fillStyle", "strokeStyle",
        "HTMLCanvasElement.addEventListener", "save", "restore"
    ]
        ...
        for ancestor in ancestors:
            try:
                if nx.get_node_attributes(G, 'type')[ancestor] == 'Script':
                    content_hash = df_graph[(df_graph['target'] == ancestor) & (df_graph['content_hash'] != "N/A")]["content_hash"]
    ...
    sc_features = [
        ascendant_script_has_eval_or_function, ascendant_script_has_fp_keyword,
...
def get_connectivity_features(G, df_graph, node):
    ...
    try:
        ...
        for parent in parents:
            try:
                if nx.get_node_attributes(G, 'type')[parent] == 'Script':
                    is_parent_script = 1
                    eval_parent = len(df_graph[(df_graph['dst'] == parent) & (df_graph['action'] == 'eval')])
                    if eval_parent > 0:
                        descendant_of_eval_or_function = 1
                    if is_parent_script & descendant_of_eval_or_function:
                        break
            except:
                continue
        ...
        connectivity_features = [
            in_degree, out_degree, in_out_degree,
