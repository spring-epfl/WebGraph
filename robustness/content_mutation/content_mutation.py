...
def perform_mutations(df, tracker_dict, output_fname, map_dir, graph_columns, change_third_partiness):

	df_graph_new = pd.DataFrame()

	try:
		visit_id = df.iloc[0]['visit_id']
		tracker_list = tracker_dict[str(visit_id)]
...
def mutation_pipeline(graph_fname, output_fname, predictions, map_dir, change_third_partiness):

	all_start = time.time()
	with open(log_fname, 'w') as f:
		f.write("Start obfuscation: " + edge_fname + "\n")
	...
	df_graph.groupby(['visit_id', 'top_level_domain']).apply(perform_mutations, tracker_dict, output_fname, map_dir, change_third_partiness)
